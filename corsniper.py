...
def sensitive_dirs(target):
            ...
            for line in wordlist:
                ...
                try:
                    ...
                    if res.status_code == 401:
                        sensitive_dir += f'Sensitive directory found: {url}\n'
                        count += 1
    ...
    if count == 0:
        return 'No sensitive directories found (No worries still exploitable)'
    else:
    ...
    try:
        with open(usr_wordlist, 'r') as wordlist:
            for line in wordlist:
                ...
                fuzz = f'{prefix}{subdomain}.{domain}'
                try:
                    res = requests.get(fuzz, timeout=7)
                    print(f'target: {fuzz}')
                    if res.status_code == 403:
                        print(f'Sensitive subdomain found: {fuzz}\n')
                        sensitive_subdomain = sensitive_subdomain + f'Sensitive subdomain found: {fuzz}\n'
                        count += 1
                except requests.RequestException:
                    continue
    except FileNotFoundError:
